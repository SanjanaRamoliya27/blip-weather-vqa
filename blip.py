# -*- coding: utf-8 -*-
"""BLIP.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1KMFubYlhKcQktUZlzgDNt85ZjIZRF022

EDA For Project
"""

import os
import zipfile
import random
import numpy as np
import pandas as pd
import seaborn as sns
import matplotlib.pyplot as plt
from PIL import Image
from collections import Counter
import cv2
from sklearn.cluster import KMeans

# Define paths
zip_path = "/content/drive/MyDrive/weather.zip"
extract_path = "/content/ML_Blip_images"

# Extract ZIP file
with zipfile.ZipFile(zip_path, 'r') as zip_ref:
    zip_ref.extractall(extract_path)

# List all image files
image_files = []
valid_extensions = ('.jpg', '.jpeg', '.png')
for root, _, files in os.walk(extract_path):
    for file in files:
        if file.lower().endswith(valid_extensions):
            image_files.append(os.path.join(root, file))

# Basic stats
total_images = len(image_files)
print(f"Total images found: {total_images}")

# Get image formats
formats = [os.path.splitext(img)[1].lower() for img in image_files]
format_counts = Counter(formats)
sns.barplot(x=list(format_counts.keys()), y=list(format_counts.values()), palette="viridis")
plt.title("Image Format Distribution")
plt.xlabel("Format")
plt.ylabel("Count")
plt.show()

# Analyze image sizes
image_sizes = []
for img_path in image_files:
    with Image.open(img_path) as img:
        image_sizes.append(img.size)  # (width, height)

widths, heights = zip(*image_sizes)

# Create a DataFrame for better visualization
df_sizes = pd.DataFrame({'Width': widths, 'Height': heights})
plt.figure(figsize=(12, 6))
sns.kdeplot(df_sizes['Width'], fill=True, label='Width', color='blue', alpha=0.5)
sns.kdeplot(df_sizes['Height'], fill=True, label='Height', color='green', alpha=0.5)
plt.title("Image Width & Height Distribution")
plt.xlabel("Pixels")
plt.ylabel("Density")
plt.legend()
plt.show()

# Aspect ratio distribution
aspect_ratios = np.array(widths) / np.array(heights)
sns.histplot(aspect_ratios, bins=30, kde=True, color='purple')
plt.title("Aspect Ratio Distribution")
plt.xlabel("Width / Height")
plt.ylabel("Count")
plt.show()

# Display random images
num_images_to_display = min(6, total_images)
random_images = random.sample(image_files, num_images_to_display)
fig, axes = plt.subplots(2, num_images_to_display // 2, figsize=(15, 7))
for ax, img_path in zip(axes.flat, random_images):
    image = Image.open(img_path).convert("RGB")
    ax.imshow(image)
    ax.set_title(os.path.basename(img_path))
    ax.axis("off")
plt.show()

# Color analysis - Extract dominant colors from sample images
def get_dominant_colors(img_path, k=5):
    img = cv2.imread(img_path)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img = img.reshape((-1, 3))
    kmeans = KMeans(n_clusters=k, random_state=42)
    kmeans.fit(img)
    return kmeans.cluster_centers_.astype(int)

random_sample = random.sample(image_files, min(10, total_images))
dominant_colors = [get_dominant_colors(img) for img in random_sample]
dominant_colors = np.array(dominant_colors).reshape(-1, 3)

plt.figure(figsize=(12, 6))
plt.imshow([dominant_colors], aspect='auto')
plt.title("Dominant Colors in Sample Images")
plt.axis("off")
plt.show()

# File size analysis
file_sizes = [os.path.getsize(img) / 1024 for img in image_files]  # Convert to KB
sns.histplot(file_sizes, bins=30, kde=True, color='red')
plt.title("File Size Distribution (KB)")
plt.xlabel("Size (KB)")
plt.ylabel("Count")
plt.show()

!pip install imagehash

# Duplicate Image Detection (Perceptual Hashing)
from imagehash import phash
hashes = {}
duplicates = []
for img_path in image_files:
    with Image.open(img_path) as img:
        img_hash = phash(img)
        if img_hash in hashes:
            duplicates.append((hashes[img_hash], img_path))
        else:
            hashes[img_hash] = img_path

print(f"Total Duplicate Images Found: {len(duplicates)}")
if duplicates:
    print("Example duplicates:", duplicates[:3])

import os
import shutil

dup_dir = "duplicates_found/"
os.makedirs(dup_dir, exist_ok=True)

for original, duplicate in duplicates:
    new_path = os.path.join(dup_dir, os.path.basename(duplicate))
    shutil.move(duplicate, new_path)
    print(f"Moved {duplicate} → {new_path}")

"""## BLIP MODEL"""

# step 1: Mount Google Drive + Set Paths
from google.colab import drive
drive.mount('/content/drive')

# CHANGE THIS TO YOUR FOLDER PATH
# Example: /content/drive/MyDrive/VQA_Project
DATASET_PATH = "/content/drive/MyDrive/YOUR_FOLDER_NAME"   # ← CHANGE THIS!
MODEL_SAVE_PATH = f"{DATASET_PATH}/blip_model.pkl"
PROCESSOR_SAVE_PATH = f"{DATASET_PATH}/blip_processor.pkl"

# Create folder if not exists
import os
os.makedirs(DATASET_PATH, exist_ok=True)

print("Google Drive mounted!")
print(f"Your files will be saved in: {DATASET_PATH}")

# step 2: Load + Save Model to Google Drive In Pickle file
import torch
import pickle
import os
from transformers import BlipProcessor, BlipForQuestionAnswering

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
print(f"Using device: {DEVICE}")

def load_blip_vqa():
    if os.path.exists(MODEL_SAVE_PATH) and os.path.exists(PROCESSOR_SAVE_PATH):
        print("Loading BLIP model from your Google Drive...")
        with open(MODEL_SAVE_PATH, "rb") as f:
            model = pickle.load(f)
        with open(PROCESSOR_SAVE_PATH, "rb") as f:
            processor = pickle.load(f)
        print("Model loaded from Drive in seconds!")
    else:
        print("First time: Downloading BLIP model (1.3 GB)... This takes 2-3 minutes")
        processor = BlipProcessor.from_pretrained("Salesforce/blip-vqa-base")
        model = BlipForQuestionAnswering.from_pretrained("Salesforce/blip-vqa-base")

        print("Saving model to your Google Drive so next time is instant...")
        model_cpu = model.to("cpu")
        with open(MODEL_SAVE_PATH, "wb") as f:
            pickle.dump(model_cpu, f)
        with open(PROCESSOR_SAVE_PATH, "wb") as f:
            pickle.dump(processor, f)
        print("Saved! Next time you open this notebook → 5-second startup!")

    model = model.to(DEVICE)
    model.eval()
    return model, processor

# Load the model
model, processor = load_blip_vqa()
print("BLIP VQA is READY!")

#step 3: Gradio App
import gradio as gr
from PIL import Image

def vqa(image, question):
    if image is None:
        return "Please upload an image"
    if not question.strip():
        return "Please write a question"

    inputs = processor(image.convert("RGB"), question, return_tensors="pt").to(DEVICE)
    with torch.no_grad():
        output = model.generate(**inputs, max_new_tokens=20)
    answer = processor.decode(output[0], skip_special_tokens=True)
    return answer.strip().capitalize()

# Example images from your Drive (optional)
example_images = []
for file in os.listdir(DATASET_PATH):
    if file.lower().endswith(('.png', '.jpg', '.jpeg')):
        example_images.append([f"{DATASET_PATH}/{file}", "What do you see?"])

with gr.Blocks(theme=gr.themes.Soft()) as demo:
    gr.Markdown("# Visual Question Answering with BLIP")
    gr.Markdown("Your model is saved in Google Drive → lightning fast next time!")

    with gr.Row():
        img = gr.Image(type="pil", label="Upload or pick from your Drive")
        question = gr.Textbox(label="Ask anything!", placeholder="How many people? What color? Is it raining?")

    btn = gr.Button("Get Answer", variant="primary")
    output = gr.Textbox(label="Answer")

    btn.click(vqa, inputs=[img, question], outputs=output)

    if example_images:
        gr.Examples(examples=example_images[:6], inputs=img)

demo.launch(share=True, debug=True)